...
import "./profile.css";
import { useGetAdsQuery } from "../search/services/searchApi"; // Assuming the API service is here
import { useGetListQuery } from "../profile/services/profileApi";
import { setFavData } from "../search/slice/FavoriteSlice";
...
const Favorite = () => {
  ...
  } = useGetListQuery({ page: currentPage }); // Fetch favorite movies list from API
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (windowHeight + scrollTop >= docHeight - 50 && !isFavoritesFetching) {
        if (
          !isFavoritesLoading &&
          !isFavoritesFetching &&
          favoriteMovies?.data?.list.length !== 0
        ) {
  ...
  if (isFavoritesFetching && currentPage === 1) {
    return (
      <div className="flex justify-center items-center h-[100vh] bg-[#161616]">
        <Loader />
      </div>
    );
  }
