...
const CommentComponent: React.FC<CommentProps> = ({ movieId, lowerDivHeight }) => {
    ...
    try {
      const response = await fetch(
        `${config.apiUrl}/movie/comments/replies`,
        {
          ...
          body: JSON.stringify({ comment_id: commentId, last_reply_id: null, limit: 5 }),
        }
      );
      const data = await response.json();
      setComments((prevComments) =>
        prevComments.map((comment) =>
          comment.id === commentId
            ? { ...comment, replies: data.data.list }
            : comment
        )
      );
    } catch (error) {
          ...
          setComments((prevComments) =>
            prevComments.map((comment) =>
              comment.id === replyingTo
                ? {
                    ...comment,
                    replies: comment.replies ? [data.data, ...comment.replies] : [data.data],
                  }
                : comment
            )
          );
  ...
  return (
    <div className="comment-section flex flex-col rounded-md p-3" style={{height: lowerDivHeight}}>
      ...
      : 
      <div className="flex justify-center items-center text-center comment-btn" style={{height: lowerDivHeight, width: '100%'}}>
      <div>
        <svg
        ...
        <p className="no_history">No Comments</p>
      </div>
    </div>
      }
