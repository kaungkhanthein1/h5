...
export const Shop = () => {
    ...
    if (1) {
      return (
        <div className="w-full flex justify-between py-3 " key={key}>
          ...
          <textarea className="outline-none border border-black/05 bg-[#FBFBFB] p-3 rounded" name="id1" placeholder="请填写您的收获邮箱，便于及时发货！(推荐使用QQ邮箱)" >
          </textarea>
        </div>
      )
    }
  ...
  return (
    <div className="container min-h-screen bg-white/90">
      <Head title="商品详情" />
      <Alert {...msg} onClose={() => { setMsg({ msg: '', show: false, navBtn: false }) }} />
            ...
            <div className="fixed z-30 inset-0 overflow-y-auto">
              <div className="flex min-h-full items-center justify-center p-4">
                <Dialog.Panel className="w-80 max-w-sm rounded bg-white p-6 flex gap-8 flex-col">
                  <Dialog.Title className="text-base font-medium">确认使用 9320 积分兑换该物品？</Dialog.Title>
      ...
      <div className="w-full py-1 mb-2 bg-white">
        <div className="w-full px-4 py-3 flex gap-2">
          <div className="w-20 h-20 rounded overflow-hidden">
            <img alt="" src={res?.image ?? ''} className="object-contain w-full h-full" />
          </div>
          ...
          <div className="flex flex-col px-4 bg-white h-full">
            ...
            >
              {controlledFields.map((field, index) => {
                return (
                  <div className={`w-full flex justify-between py-3 ${field.type === 'textarea' ? 'flex-col gap-2' : ''}`}>
