...
const Main = () => {
  ...
  // Trigger search when query comes from URL parameters (initial load)
  useEffect(() => {
    if (initialQuery) {
      handleSearch(); // Automatically trigger search with initial query
    }
  ...
  useEffect(() => {
    if (data) {
      if (currentPage === 1) {
        ...
        setSecAgain(false);
        setNoDataFound(data.data.list.length === 0); // Check if no data is found
      } else {
        setMovies((prevMovies) => [...prevMovies, ...data.data.list]);
        setSecAgain(false);
      }
    }
  }, [data, secAgain]);
